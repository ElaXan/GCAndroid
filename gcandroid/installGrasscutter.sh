...
Install_Grasscutter_process() {
    ...
    if [[ $Install_Grasscutter_Resources = "tamilpp25" ]]; then
        if [[ $Install_Grasscutter_Resources_Version = "3.2" ]]; then
            ...
            run_Program() {
                wget https://github.com/tamilpp25/Grasscutter_Resources/archive/refs/heads/3.0.zip -O zResources.zip &>$HOME/zerr.log
                errCode=$?
                log "$errCode"
            }
        elif [[ $Install_Grasscutter_Resources_Version = "2.8" ]]; then
            run_Program() {
                wget https://github.com/tamilpp25/Grasscutter_Resources/archive/refs/heads/2.8.zip -O zResources.zip &>$HOME/zerr.log
                errCode=$?
                log "$errCode"
            }
        fi
        ...
        if [[ $Install_Grasscutter_Resources_Version = "3.2" ]]; then
            ...
            run_Program() {
                wget https://gitlab.com/yukiz/GrasscutterResources/-/archive/3.0/GrasscutterResources-3.0.zip -O zResources.zip &>$HOME/zerr.log
                errCode=$?
                log "$errCode"
            }
        elif [[ $Install_Grasscutter_Resources_Version = "2.8" ]]; then
            run_Program() {
                wget https://gitlab.com/yukiz/GrasscutterResources/-/archive/2.8/GrasscutterResources-2.8.zip -O zResources.zip &>$HOME/zerr.log
                errCode=$?
                log "$errCode"
            }
        elif [[ $Install_Grasscutter_Resources_Version = "2.7" ]]; then
            run_Program() {
                wget https://gitlab.com/yukiz/GrasscutterResources/-/archive/2.7/GrasscutterResources-2.7.zip -O zResources.zip &>$HOME/zerr.log
                errCode=$?
                log "$errCode"
            }
        elif [[ $Install_Grasscutter_Resources_Version = "2.6" ]]; then
            run_Program() {
                wget https://gitlab.com/yukiz/GrasscutterResources/-/archive/2.6/GrasscutterResources-2.8.zip -O zResources.zip &>$HOME/zerr.log
                errCode=$?
                log "$errCode"
            }
        fi
    ...
    run_Program() {
        unzip $HOME/zResources.zip &>$HOME/zerr.log
        errCode=$?
        log "$errCode"
    }
    run_Program &
    ...
    sleep 1s
    run_Program() {
        mv $HOME/Grasscutter*Resources*/Resources $HOME/Grasscutter/resources &>$HOME/zerr.log
        errCode=$?
        log "$errCode"
    }
    run_Program &
